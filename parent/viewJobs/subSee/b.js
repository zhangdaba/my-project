    ...
    for (let k in xyPoints) {
      for (let i in sobps) {
        if (xyPoints[k].sobp == sobps[i]) {
          xyPoints[k].abX = (xyPoints[k].abX) * (2481 / A4W * CodePoint) * ratio - dots.startX - offsetX
          // 271 ：第一页最下部分和第二页最上部分为空白, 故做删除后拼接处理，271=297-2△y
          xyPoints[k].abY = (xyPoints[k].abY) * ( 3508 / A4H * CodePoint) * ratio - dots.startY + i * (271 * (3508 / A4H)) - offsetY
